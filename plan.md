...
demo / proof / hardware proof packages は `examples/` に置きます。
...
```rust
appkit::run::<
    site::Local<image::LinuxControl>,
    heterogeneous_control::Control,
>(
    artifacts.for_image::<image::LinuxControl>(),
);
...
pub trait WasiGuestImage<C: appkit::Capsule>: appkit::LogicalImage<C> {
    fn wasi_guest_storage<'guest, const ROLE: u8>() -> appkit::WasiGuestStorage<'guest>;
}
```

`WasiGuestImage` は `type Artifact = appkit::WasiImage<'_>` の logical image
だけが実装します。`type Artifact = appkit::NoWasi` の logical image は
`WasiGuestImage` を実装せず、WASI guest storage hook も持たず、guest
storage を lease しません。
...
- embedded scheduler loop must not inline a role localside future poll into the
  runner loop. A logical image runner stores the future in the bounded arena and
  polls it through a stable, non-inlined poll boundary. This keeps scheduler
  state and role-future temporaries separate on small bare-metal stacks.
- embedded scheduler storage is a `LogicalImage` storage fact, not a global
  `Role<0>` / `Role<1>` table. `Role<2>` and later must never alias role1
  future, waker, or context storage. A role slice is accepted only if the
  logical image storage can materialize that slice without role-number fallback.
- bare-metal `appkit::run` owns one long-lived role task. If a physical firmware
  contains multiple roles, it contains multiple logical images and chooses the
  matching `run::<LogicalImage, Capsule>()` from site-local entry code. appkit
  must not provide a hidden co-located no_std scheduler that polls peer roles
  inside one logical image.
...
workspace/
  ...
  examples/heterogeneous-split-example/ # one Capsule, multiple logical-image packages
  guest/hibana-wasip1-guest/         # generic WASI P1 guest helper crate
  guest/wasip1-programs/             # generic WASI P1 guest programs
```

physical artifact package は、必要なら検証や配置ごとに複数の bin target
を持ってよいです。package-local `lib.rs` は reset / carrier / markers /
boot2 / clock setup / small execution helpers だけを共有します。各 bin は、その検証で使う
Capsule / raw hibana choreography / Localside / ChoreoFsObject を直接持ち、
source を開けば検証内容が一目で読める形にします。共通 Capsule を
`lib.rs` に隠しません。

Example physical artifacts own their hardware boot and frequency environment.
Do not pull a board boot crate into Baker or into core. Baker's RP2040 firmware
keeps the W25Q080 boot2 block and Pico SDK-equivalent clock setup in
`examples/baker-firmware/src/lib.rs`: XOSC 12MHz, PLL_SYS VCO 1500MHz with
postdiv 6/2, `clk_sys` 125MHz from PLL_SYS, `clk_peri` from `clk_sys`, and
watchdog tick generation at 1MHz. This is example-local hardware materialization,
not a `site` or `appkit` concept.

WASI P1 guest-side code is not an example Capsule.

`guest/hibana-wasip1-guest` is a generic guest helper crate. It may expose
small helpers for ChoreoFS path/object facts and clock waits, but it must not
contain Baker, board, LED, transport, network, or device-specific meaning.

`guest/wasip1-programs` contains generic WASI P1 guest programs for the VM and
WASI allowlist. It must not contain Baker LED programs, socket programs, or
example-specific device meaning.

The WASI P1 guest build gate must validate the current expected artifact set,
not whatever stale `.wasm` files happen to remain in `target/`. It cleans the
shared WASI target release directory, builds the owned guest packages, compares
the produced `.wasm` basenames against an explicit expected list, and rejects
P2/WIT/Component surfaces in every produced artifact. It must not reject an
artifact merely because a WASI P1 static import table mentions an unsupported
Preview 1 function such as `sock_*`; static imports are not choreography
authority, and unsupported calls become terminal runtime evidence only if the
guest actually reaches them.

Example-specific guest programs live with the example that owns their meaning.
For Baker hardware, Baker-specific WASI guest helpers and guest programs live
under `examples/baker-firmware/wasip1/guest`. The package-local `lib.rs` owns
Baker LED path/preopen facts and other Baker guest helpers; each bin owns one
guest program such as the ChoreoFS traffic cycle.
...
- static import table は admission authority ではない
- unsupported import は実際に呼ばれた時点で typed reject / terminal fault にする
...
active capacity にはしません。ただし static import table に存在するだけでは
reject しません。std WASI P1 artifact は未使用の `environ_*` / `proc_exit` などを
持つことがあるため、static import table を choreography 前の authority にしては
いけません。実際に guest が import を呼んだ時点だけが protocol boundary です。
...
```text
static WASI import table
  -> implementation/load evidence only
  -> not protocol authority
  -> not choreography admission condition

Wasm instruction execution
  -> WASI P1 import boundary
  -> typed EngineReq
  -> Endpoint / carrier
  -> projected choreography frontier
```

Only the actual import call crosses the authority boundary. If the projected
frontier does not admit that typed `EngineReq`, the current session generation
faults. No earlier static validation may authorize or reject protocol progress.
...
WASI guest は `WasiGuestImage` / site-local storage facts が渡す in-place arena lease にだけ構築します。
...
The VM may recognize `proc_raise` as a WASI P1 import name for diagnostics, but
the static import table is not admission authority. If a guest actually calls
`proc_raise`, the call becomes terminal unsupported evidence. It must not become
a runtime `Call`, must not be fake-successed, and must not be mapped into hibana
abort / fence / control choreography. WASI `proc_raise` is local POSIX
self-signal compatibility; hibana abort / fence / topology control stays
explicit raw choreography.

`sched_yield` is deliberately unsupported for now.
The VM may recognize `sched_yield` as a WASI P1 import name for diagnostics, but
the static import table is not admission authority. If a guest actually calls
`sched_yield`, the call becomes terminal unsupported evidence. It must not become
a runtime `Call`, must not be fake-successed, and must not be mapped into hibana
resolver / route frontier / policy-yield internals. WASI `sched_yield` is a local
POSIX scheduler hint; hibana resolver semantics stay explicit raw choreography
and policy configuration. `EngineReq::Yield` / `EngineRet::Yielded` must not be
used as a WASI `sched_yield` compatibility path; if they have no separate
choreography use, they are deletion candidates.

`sock_*` is deliberately unsupported in hibana-pico core.
The VM may recognize WASI P1 socket import names for diagnostics, but appkit and
the VM must not reject the static import table. If a guest actually calls
`sock_*`, the call becomes terminal unsupported evidence. It must not become
...
import names for diagnostics, but the static import table is not admission
authority. If a guest actually calls one of them, the call becomes terminal
unsupported evidence. ChoreoFS is a bounded path/object fact resolver, not a host
...
`fd_filestat_set_times` may be recognized for diagnostics, but the static import
table is not admission authority. If a guest actually calls one of them, the call
becomes terminal unsupported evidence. They do not clarify choreography
authority; they imply a POSIX file abstraction that hibana-pico does not provide.
...
therefore terminal unsupported evidence if called.
...
### Test Program Quarantine

Production VM files must not contain demo program byte arrays.
Test program modules belong in `#[cfg(test)]` modules or external test assets.

Deletion targets:

- `DEMO_*` guest byte arrays
- traffic light byte programs
- bad route demo byte programs
- sleep/timer demo byte programs
...
appkit linked metadata storage is bounded. If projected labels, policies,
controls, loop-control heads, or WASI completion metadata exceed that linked
capacity, appkit must reject attach/run as a capacity mismatch. It must not
silently truncate metadata and then continue with an incomplete authority view.
...
- transport `open(local_role, session_id, lane)` receives and preserves the
  logical lane selected by attach; wrappers must not drop the lane argument
- SIO writes the lane into carrier frame metadata and demultiplexes by lane
  before yielding payload bytes
- SIO `poll_send` / `poll_recv` are non-blocking carrier polls; they may stage
  partial frames in carrier state and return `Poll::Pending`, but they must not
  spin inside FIFO push/pop loops
...
- do not replace ownership with an atomic mailbox just because the target has
  atomics
- read-modify-write atomics are a second-line primitive for state that is truly
  shared concurrently and cannot be made single-owner without adding more
  complexity
...
- route-observation hints are keyed by lane and frame label; frame label alone
  is not enough when projected branches can share a wire label on different lanes
- a carrier hint may wake a waiter or prevent parking on the wrong lane, but it
  must not mint a route continuation by itself
...
### Passive Route Evidence Gate

Dynamic branch selection is resolver authority at an explicit projected route
point. A passive projection can reach a structural arm entry before the
controller's route decision frame or materializing payload is observed. That
state is not progress, but it is also not a protocol violation by itself.

Required:

- passive `offer()` waits for projected route evidence
- passive route evidence is lane-scoped; lane + frame label may be preview
  evidence, but frame label alone is never route authority
- passive current-arm position without selected / preview route evidence returns
  no continuation
- passive current-arm position must not restore or synthesize selected arm state
- controller-side missing route authority remains fail-closed
- route evidence must come from projected resolver / route-control / payload
  metadata, not transport hints or label guesses

Forbidden:

- repairing missing route state from cursor position
- treating passive route-evidence absence as hidden progress
- poisoning the generation merely because a passive projection is waiting at an
  arm entry
- using transport lane hints or payload labels as route authority
- accepting a same-label frame on a different lane as evidence for this lane
- treating `recv_frame_hint` as payload receive or as reusable branch evidence

Regression proof:

- hibana `passive_dynamic_offer_without_route_evidence_waits_instead_of_faulting`
- hibana `passive_current_arm_position_without_runtime_route_state_waits`
- hibana `test_transport_demuxes_lane_and_drains_route_hint`
- hibana semantic gate for lane-preserving transport hints
- Baker `preview-probe`, which proves SIO route-observation hints remain preview evidence
- Baker `timer-route`, which exercises resolver-selected route progress over the
  real RP2040 SIO carrier
...
If official metadata exceeds appkit linked storage, reject the logical image as
a linked implementation capacity mismatch. Silent truncation is forbidden.
...
- WASI guest storage is supplied by `WasiGuestImage` / site-local storage facts as an in-place arena lease.
- `NoWasi` logical images do not implement `WasiGuestImage` and do not expose dummy storage hooks.
...
- `proc_raise` is unsupported POSIX self-signal compatibility; it may exist in a static import table, but if called it becomes terminal unsupported evidence and is never mapped to abort / fence / control choreography.
- `sched_yield` is unsupported POSIX scheduler-hint compatibility; it may exist in a static import table, but if called it becomes terminal unsupported evidence and is never mapped to hibana resolver / route frontier internals.
- `sock_*` is unsupported POSIX socket compatibility in core; it may exist in a static import table, but if called it becomes terminal unsupported evidence and never becomes raw socket authority.
...
- Production VM files do not contain demo program byte arrays.
