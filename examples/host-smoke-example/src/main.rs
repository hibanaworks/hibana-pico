...
struct Smoke;
struct SmokePlacement;
struct SmokeLocal;
struct SmokeArtifacts;
const HOST_SMOKE_CARRIER: appkit::CarrierKind = appkit::CarrierKind::new(1);

#[cfg(feature = "wasm-engine-core")]
static HOST_SMOKE_WASI_GUEST_ARENA: appkit::WasiGuestArena = appkit::WasiGuestArena::empty();
...
impl appkit::Capsule for Smoke {
    type Universe = DefaultLabelUniverse;
    type Placement = SmokePlacement;
    type Local = SmokeLocal;
    type Report = usize;
...
impl appkit::Placement<Smoke> for SmokePlacement {
...
impl<I> appkit::ArtifactForImage<Smoke, I> for SmokeArtifacts
where
    I: appkit::LogicalImage<Smoke, Artifact = appkit::NoWasi>,
{
...
impl appkit::LogicalImage<Smoke> for site::Local<image::Composite> {
    ...
    const CARRIER: appkit::CarrierKind = HOST_SMOKE_CARRIER;
    ...
    fn wasi_guest_storage<'guest, const ROLE: u8>() -> appkit::WasiGuestStorage<'guest> {
        ...
        HOST_SMOKE_WASI_GUEST_ARENA.storage()
    }
}

static ARTIFACTS: SmokeArtifacts = SmokeArtifacts;

fn main() {
    let report = appkit::run::<site::Local<image::Composite>, Smoke>(
        ARTIFACTS.for_image::<site::Local<image::Composite>>(),
    );
